...
fn apply_jokers(t: Type, joker_count: u8) -> Type {
    match joker_count {
        0 => t,
        5 => Type::Five,
        1 => {
        ...
        2 => {
            match t {
                Type::Pair => Type::Three,
                Type::TwoPair => Type::Four,
                Type::House => Type::Five,
                _ => panic!("Should not have gotten here"),
            }
        }
        3 => {
...
fn hand_to_type(hand: &str) -> Type {
    ...
    let initial_type = get_type_without_jokers(&counts);
    apply_jokers(initial_type, *joker_count)
}

fn get_type_without_jokers(counts: &HashMap<char, u8>) -> Type {
