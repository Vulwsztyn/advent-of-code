...
fn get_numbers(lines: &Vec<&str>, index: usize) -> Vec<u128> {
    ...
    let line_without_spaces = lines[index].replace(" ", "");
    println!("{} {}", &lines[index], line_without_spaces);
    number_regex.find_iter(&line_without_spaces).map(
        |(start, end)| {
            line_without_spaces[start..end].parse::<u128>().unwrap()
        }
    ).collect::<Vec<u128>>()
